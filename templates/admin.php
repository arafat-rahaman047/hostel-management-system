...
<ul class="nav nav-tabs mb-3" id="paymentTabs">
    <li class="nav-item"><a
            class="nav-link <?php echo (!isset($_GET['pstatus']) || $_GET['pstatus'] === 'Pending') ? 'active' : ''; ?>"
            href="?pstatus=Pending">⏳ Pending</a></li>
    <li class="nav-item"><a class="nav-link <?php echo (($_GET['pstatus'] ?? '') === 'Verified') ? 'active' : ''; ?>"
            href="?pstatus=Verified">✅ Verified</a></li>
    <li class="nav-item"><a class="nav-link <?php echo (($_GET['pstatus'] ?? '') === 'Rejected') ? 'active' : ''; ?>"
            href="?pstatus=Rejected">❌ Rejected</a></li>
    <li class="nav-item"><a class="nav-link <?php echo (($_GET['pstatus'] ?? '') === 'all') ? 'active' : ''; ?>"
    ...
    <table class="table table-hover align-middle">
        ...
        <tbody>
            <?php if ($payments && $payments->num_rows > 0):
                        ...
                        <td class="text-center">
                            <?php if ($p['status'] === 'Pending'): ?>
                                <a href="../../api/verify_payment.php?id=<?php echo $p['payment_id']; ?>&action=verify"
                                    class="btn btn-success btn-sm" onclick="return confirm('Verify this payment?')">✅ Verify</a>
                                <a href="../../api/verify_payment.php?id=<?php echo $p['payment_id']; ?>&action=reject"
                                    class="btn btn-outline-danger btn-sm" onclick="return confirm('Reject this payment?')">❌
                                    Reject</a>
                ...
                <?php endwhile; else: ?>
                <tr>
                    <td colspan="9" class="text-center py-4 text-muted">No <?php echo $pstatus; ?> payments found.</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
